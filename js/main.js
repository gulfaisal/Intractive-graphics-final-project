...
function init() {
  ...
  if (hasPointerLock) {
    ...
    alert(
      "Mine3js requires pointer lock and your browser doesn't seem to have it :/"
    );
  }
...
function initMaterials() {
  ...
  mat.grass = new THREE.MeshLambertMaterial({
    map: textureGrass,
    ambient: 0xbbbbbb,
    vertexColors: THREE.VertexColors,
  });
  mat.dirt = new THREE.MeshLambertMaterial({
    map: textureDirt,
    ambient: 0xbbbbbb,
    vertexColors: THREE.VertexColors,
    shading: THREE.FlatShading,
  });
  mat.grassDirt = new THREE.MeshBasicMaterial({
...
function physics(dt) {
  ...
  camera.vy = Math.max(camera.vy, -5);
